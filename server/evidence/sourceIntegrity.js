...
function makeSource({ videoId, items, unit, language, durationSeconds, kind = 'youtube_captions' }) {
  ...
  const content = { version: EVIDENCE_VERSION, video_id: videoId, kind, language: lang,
    ...
    duration_seconds: durationSeconds, segments };
...
function validateEvidence(item, source, reportLanguage = source.language) {
  ...
  if (!Array.isArray(ids) || !ids.length || ids.length > 6 || new Set(ids).size !== ids.length) {
    throw new SourceError('EVIDENCE_UNSUPPORTED', 'Ein Beleg hat keine eindeutigen Quellsegmente.');
  }
  const positions = ids.map(id => source.segments.findIndex(segment => segment.id === id));
  if (positions.some((position, index) => position < 0 || (index > 0 && position !== positions[index - 1] + 1))) {
    throw new SourceError('EVIDENCE_UNSUPPORTED', 'Belegsegmente sind unbekannt oder nicht zusammenhängend.');
  }
  const segments = positions.map(index => source.segments[index]);
...
function validateReport(data, source, reportLanguage = source.language) {
  ...
  const companies = data.companies.map(company => {
    ...
    return { ...company, evidence: company.evidence.map(item => validateEvidence(item, source, reportLanguage)),
...
module.exports = { SourceError, makeSource, validateEvidence, validateReport, normalText, languageCode, assertLanguage, digest };
